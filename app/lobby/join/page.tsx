...
export default function JoinLobbyPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError("");
    const cleanCode = code.trim().toUpperCase().slice(0, 5);
    if (cleanCode.length !== 5) {
      setError("Kod 5 karakter olmalı");
      return;
    }
    if (!nickname.trim()) {
    ...
    try {
      const res = await fetch("/api/lobby/join", {
        ...
        body: JSON.stringify({ code: cleanCode, nickname: nickname.trim(), avatarId }),
      });
      const data = await res.json();
      if (!res.ok) {
        setError(data.error ?? "Katılınamadı");
        return;
      }
  ...
  return (
    <div className="page-bg min-h-screen flex flex-col items-center justify-center px-4 py-12">
      ...
      <div className="mx-auto w-full max-w-md">
        ...
        >
          <label className="block text-sm font-medium text-gray-400">Lobi kodu</label>
          <input
            type="text"
            value={code}
            onChange={(e) => setCode(e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, "").slice(0, 5))}
            maxLength={5}
            placeholder="örn. HDFQS"
            className="mt-2 w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3.5 text-center text-xl font-mono tracking-[0.35em] text-white placeholder-gray-500 transition focus:border-indigo-500/80 focus:outline-none focus:ring-2 focus:ring-indigo-500/30"
          />
          <label className="mt-4 block text-sm font-medium text-gray-400">Avatar seç</label>
          <div className="mt-2">
          ...
          >
            {loading ? "Katılıyor…" : "Lobiye Katıl"}
          </motion.button>
        </motion.form>
      </div>
    </div>
